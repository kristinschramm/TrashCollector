...
namespace TrashCollector.Controllers.Api
{
...

    {
            ...
            {
                var employeeDto = _context.Employees.SingleOrDefault(e => e.Id == id);
            ...
            var currentZipCode = employeeDto.ZipCodeId;
            string todaysDayName = DateTime.Now.DayOfWeek.ToString();

            var customersToday = customerDtos.Where(c => (employeeDto.ZipCodeId == c.ZipCodeId && c.Day.Name == todaysDayName)).ToList();
                return Ok(customersToday);
            }
